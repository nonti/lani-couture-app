...
const Carousel = ({ images, autoSlide = true, autoSlideInterval = 4500 }) => {
  ...
  return (
    <>
    <div className='relative w-full max-w-6xl mx-auto rounded-2xl'>
      <div className='overflow-hidden relative h-72'>
        {images.map((image, index) => (
          ...
          >
            <img src={image} alt={`Slide ${index}`} className='w-full h-72' />
          </div>
        ))}
      </div>
      ...
      <button
        ...
        onClick={prevSlide}
      >
        <IoIosArrowDropleftCircle className='bg-white rounded-full text-pink-500'/>
      </button>
      <button
        className='absolute top-1/2 right-0 transform -translate-y-1/2  text-white p-2'
        onClick={nextSlide}
      >
        <IoIosArrowDroprightCircle className='bg-white rounded-full text-pink-500'/>
      </button>
